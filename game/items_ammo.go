...
func (i *Ammo) FullDescription(colorCode string) string {
	basicRows := i.GenericItem.fullDescriptionRows()

	appendIfNotOne := func(value float64, name string) {
		if value != 1 {
			basicRows = append(basicRows, fxtools.NewTableRow(name, fmt.Sprintf("%.2f", value)))
		}
	}
	...
	appendIfNotOne(i.DamageFactor, "Damage Factor")
	appendIfNotOne(i.ConditionFactor, "Condition Factor")
	appendIfNotOne(i.SpreadFactor, "Spread Factor")

	appendIfNotZero(i.DTModifier, "DT Modifier")
	appendIfNotZero(i.BonusRadius, "Bonus Radius")

	basicRows = append(basicRows, fxtools.NewTableRow("Rounds in Magazine", fmt.Sprintf("%d", i.RoundsInMagazine)))

	for tag, damage := range i.BonusDamageAgainstActorWithTags {
		basicRows = append(basicRows, fxtools.NewTableRow(fmt.Sprintf("Bonus Damage vs. %s", tag.String()), fmt.Sprintf("%+d", damage)))
	}
...
func (i *Ammo) LongNameWithColors(colorCode string) string {
