...
func (g *GameState) ManualMovePlayer(direction geometry.CompassDirection) {
    ...
    if player.HasFlag(foundation.IsConfused) && rand.Intn(5) != 0 {
    ...
    if _, exists := g.gridMap.TryGetObjectAt(newPos); exists {
...
func (g *GameState) afterPlayerMoved() {
    ...
    g.exploreMap()
    g.updateDijkstraMap()
    g.triggerTileEffectsAfterMovement(g.Player, g.Player.Position())
}
