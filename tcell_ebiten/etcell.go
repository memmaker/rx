...
func (et *etcell) SetClipboard(bytes []byte) {

}

func (et *etcell) GetClipboard() {
...
func NewGameScreen(font_face text.Face) GameScreen {
    ...
    et := &etcell{
        ...
        forcedFallback:  make(map[rune]bool),
        italic_skew:     -0.108,
...
func (et *etcell) SetFallbackFont(face text.Face) {
    et.fallbackFace = face
}

func (et *etcell) mainFontHasGlyph(r rune) bool {
    if _, forced := et.forcedFallback[r]; forced {
        return false
    }
    return true
}
...
func (et *etcell) Layout(outsideWidth, outsideHeight int) (screenWidth, screenHeight int) {
    width, height := et.LayoutF(float64(outsideWidth), float64(outsideHeight))
...
func (et *etcell) LayoutF(outsideWidth, outsideHeight float64) (screenWidth, screenHeight float64) {
    ...
    cellSizeX := float64(et.cell_size.X)
    cellSizeY := float64(et.cell_size.Y)
    grid_size := image.Point{
        X: int(outsideWidth / cellSizeX),
        Y: int(outsideHeight / cellSizeY),
    }

    screenWidth = float64(grid_size.X) * cellSizeX
    screenHeight = float64(grid_size.Y) * cellSizeY

    if !grid_size.Eq(et.grid_size) {
        et.grid_lock.Lock()
        et.grid_size = grid_size
        et.grid = make([]cell, et.grid_size.X*et.grid_size.Y)
        et.grid_image = ebiten.NewImage(int(screenWidth), int(screenHeight))
        et.grid_image.Fill(color.Transparent)
        et.blink_image = ebiten.NewImage(int(screenWidth), int(screenHeight))
...
func (et *etcell) SetCursorStyle(cs tcell.CursorStyle, color ...tcell.Color) {
    et.cursor_style = cs
    if len(color) > 0 {
        et.cursor_color = color[0]
    } else {
        et.cursor_color = tcell.ColorWhite
    }
}
...
func (et *etcell) Show() {
    ...
    for y := 0; y < et.grid_size.Y; y++ {
        ...
        for x := 0; x < et.grid_size.X; x++ {
            ...
            runeDrawFace := et.face
            if et.fallbackFace != nil && !et.mainFontHasGlyph(cell.Rune) {
                runeDrawFace = et.fallbackFace
            }
            text.Draw(dst, string(cell.Rune), runeDrawFace, &text_opts)

            // Draw the combining runes
            if len(cell.Combining) > 0 {
                text.Draw(dst, string(cell.Combining), runeDrawFace, &text_opts)
            }
...
func (et *etcell) ForcedFallbacks(r []rune) {
    for _, runeChar := range r {
        et.forcedFallback[runeChar] = true
    }
}
func (et *etcell) RegisterRuneFallback(r rune, str string) {
    et.forcedFallback[r] = true
}
