...
func (k *KeyPad) drawKeyPad(screen tcell.Screen, x int, y int, width int, height int) {
	...
	// draw special string below the keypad
	if k.specialString != "" {
		specialStyle := tcell.StyleDefault.Background(tcell.ColorDarkGreen).Foreground(tcell.ColorWhite).Bold(true)
		for i := 0; i < width; i++ {
			screen.SetContent(x+i, y+height, ' ', nil, specialStyle)
		}
		for i, r := range []rune(k.specialString) {
			screen.SetContent(x+i, y+height, r, nil, specialStyle)
		}
	}
}
...
func (k *KeyPad) SetSpecialString(specialString string) {
	k.specialString = specialString
}
