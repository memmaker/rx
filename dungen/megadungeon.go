...
func (c *MegaDungeonGenerator) chooseRandom(other map[Region][]geometry.Point) (Region, geometry.Point) {
	var flatList []util.Tuple[Region, geometry.Point]

	for reg, pos := range other {
		for _, p := range pos {
			flatList = append(flatList, util.NewTuple(reg, p))
		}
	}
	if len(flatList) > 0 {
...
func (c *MegaDungeonGenerator) isConnectingRegions(emptyMap *DungeonMap, pos geometry.Point) (util.Tuple[Region, Region], bool) {
	if !emptyMap.IsWallAt(pos) {
		return util.Tuple[Region, Region]{}, false
	}
	// check north/south
...
func (c *MegaDungeonGenerator) areInDifferentRegions(emptyMap *DungeonMap, posOne geometry.Point, posTwo geometry.Point) (util.Tuple[Region, Region], bool) {
	...
	return util.NewTuple(northernRegion, southernRegion), true
}
