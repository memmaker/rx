...
func GraphDialogue(rootDir string, handler convo.DialogueHandler, filename string, hideBackLinksToNode string) {
...
func (dc *DialogueChecker) CheckSingleFile(filename string, handler convo.DialogueHandler) DialogueReport {
	return DialogueReportFromFile(dc.rootDir, filename, handler)
}

func (dc *DialogueChecker) CreateReports(handler convo.DialogueHandler) []DialogueReport {
	...
	for _, entry := range dir {
		...
		report := DialogueReportFromFile(dc.rootDir, entry.Name(), handler)
...
func appendFromNodes(nodes map[string]NodeInfos, records map[string]convo.ConversationNode) map[string]NodeInfos {
