...
func LoadDerivedBaseValues(m map[string]string) {
	for derivedStat, baseValue := range m {
		expr, err := govaluate.NewEvaluableExpressionWithFunctions(baseValue, standardFuncs)
		if err != nil {
			panic(err)
		}
...
func LoadSkillDifficulties(diffs map[string]string) {
	for skillName, modExpr := range diffs {
		...
		expr, err := govaluate.NewEvaluableExpressionWithFunctions(modExpr, standardFuncs)
		if err != nil {
			panic(err)
		}
